# SparkClaw Review Note (KO)

이 저장소는 Aural Reality의 전체 제품 저장소가 아니라, SparkClaw와 초기
리뷰어에게 공개해도 되는 범위만 정리한 공개 증거 채널입니다.

전체 비공개 저장소에는 미공개 앱 코드, Creator Tool, source audio, streaming
infrastructure, tester/evidence boundary, 내부 운영 문서가 포함되어 있어 그대로
공개하지 않습니다. 대신 이 저장소에는 기술 방향, 대표 코드 패턴, AI-native 개발
운영 방식, 검증 스냅샷을 공개 가능한 형태로 정리했습니다.

## 5분 리뷰 동선

1. [README](../README.md)를 읽습니다.
2. [Technical System](technical-system.md)에서 런타임 구조를 확인합니다.
3. [AI-Native Workflow](ai-native-workflow.md)에서 AI agent 활용 방식을 확인합니다.
...
청각 연출로 바꾸는 런타임 엔진과 검증 시스템**입니다.

Aural Reality는 단순한 오디오 재생 앱이 아닙니다. 사용자가 실제 장소를 걷고
있을 때 날씨, 시간, 위치 존, 이동 상태, 청취 안전 상태를 바탕으로 작가가 의도한
사운드 레이어를 열고 닫는 청각 작품 플랫폼입니다. 예를 들어 비 오는 날 특정
장소에서만 열리는 7분짜리 청각 작품처럼, 현실 조건을 작품의 장면과 사운드
연출로 번역하는 구조를 만들었습니다.

기술적으로 어려웠던 이유는 입력값이 불안정하기 때문입니다. GPS는 존 경계에서
...
이를 해결하기 위해 상태 머신, zone hysteresis, 작가 정의 규칙, fail-closed
evidence gate를 사용했습니다. 존 경계에서는 바로 상태를 바꾸지 않고 enter/exit
여유 폭을 두어 채터링을 줄였고, 날씨와 움직임, 누적 청취 상태는 작가가 미리
정의한 경계 안에서만 사운드 레이어를 바꾸도록 했습니다. 즉 AI가 임의로 감정이나
장소의 의미를 해석해 콘텐츠를 생성하는 방식이 아니라, 안전하고 예측 가능한
규칙 안에서 현실 맥락을 연출로 바꾸는 구조입니다.

또 하나 중요하게 설계한 것은 **검증되지 않은 것을 완료된 것처럼 말하지 못하게
하는 시스템**입니다. 로컬 테스트 통과가 실제 Android 기기 재생 검증을 뜻하지
않고, 시뮬레이터 결과가 field test acceptance를 뜻하지도 않습니다. 그래서
...
Aural Reality의 현재 AI 최적화는 "런타임에서 대형 언어모델을 호출해 즉석
...
2026-06-25 기준 비공개 workspace에서 확인한 공개 가능 스냅샷은 다음과 같습니다.
...
acceptance, listener streaming approval, field-test acceptance, launch_ready catalog
count를 의미하지 않습니다.

## 공개하지 않는 것

- private source audio / stems / mix sessions
- raw location, route, sensor, weather, microphone, tester, participant data
- token, signed URL, endpoint, bucket, project ref, secret
- full mobile app / Creator Tool / Worker / Supabase implementation
- internal commercial strategy, legal notes, private incident notes, live ops runbook

이 경계 때문에 공개 repo는 전체 코드를 덜 보여주는 것이 아니라, 심사자가 봐도
